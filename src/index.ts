...
- **Self-healing**: When you detect workflow problems, create a task to fix the root cause rather than working around it.
...
**Use builders** for well-defined tasks — they are faster and more predictable. **Use leads** for complex tasks requiring scouting or decomposition, but give them clear descriptions to avoid stalling.
...
| \`grove task update <id> -s <status>\` | Update task status |
| \`grove task list [-s <status>] [-a]\` | List tasks |
| \`grove spawn <task-id> -n <name> -c <cap>\` | Spawn agent (builder/scout/reviewer/lead) |
| \`grove status\` | Show all agents with tree view |
| \`grove merge --all [--review]\` | Merge all completed branches (optionally spawn reviewer) |
...
| \`grove feed [-f]\` | Event feed (use -f to follow live) |
| \`grove metrics [--agent <name>]\` | Tool usage metrics |
